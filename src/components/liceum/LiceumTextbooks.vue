<template>
    <main class="liceum-textbooks-container">
        <v-btn
          v-if="isAuth"
        ...
        >
          <v-tab
            v-for="(item, key) in liceumClasses"
            :key="key"
          >
        ...
        <v-tabs-items v-model="tab">
          ...
          >
            <div
              v-if="isAuth && computedLiceumClasses.length"
              class="edit-btn"
            >
              <v-btn
  ...
  export default {
      ...
      data() {
        return {
          ...
          liceumClasses: [],
          tab: null,
          liceumClassesType: 'liceum-classes',
          editorDialog: false,
          classDialog: false,
          loading: false,
          form: {
      ...
      computed: {
        ...
        computedLiceumClasses() {
          return Object.values(this.liceumClasses);
        },
      },
      watch: {
        tab(newVal) {
          if (this.computedLiceumClasses.length) {
            this.currentTextbooksListType = this.computedLiceumClasses[newVal].id;
            this.getLiceumTextbooksList();
          } else {
      ...
      methods: {
        getLiceumClasses() {
            ...
            const dbRef = ref(db, this.liceumClassesType);
            
            onValue(dbRef, (snapshot) => {
                this.liceumClasses = snapshot.val() || [];
            }, {
                onlyOnce: false
            });
        ...
        addNewClass() {
            ...
            const db = getDatabase();
            const list = ref(db, this.liceumClassesType);
        ...
        editClass() {
            const db = getDatabase();
            const dbRef = ref(db, `${this.liceumClassesType}/${this.form.key}`);
            const data = {
                name: this.form.name,
            }
